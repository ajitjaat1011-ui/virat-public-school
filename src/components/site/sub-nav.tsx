...
export function SubNav({ title, links }: SubNavProps) {
  ...
  return (
    <aside className="glass-panel w-full min-w-0 max-w-full rounded-none border-t-2 border-t-[var(--accent)] p-4 lg:sticky lg:top-28 lg:self-start">
      ...
      <nav className="grid w-full min-w-0 grid-cols-2 gap-2 lg:grid-cols-1" aria-label={`${title} section navigation`}>
        {links.map((link) => {
          ...
          return <Link key={link.href} href={link.href} aria-current={active ? "page" : undefined} className={cn(
            "flex min-h-12 min-w-0 items-center rounded-none border px-3 py-2 text-sm leading-snug transition-colors",
